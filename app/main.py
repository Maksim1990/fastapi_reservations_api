...
@app.post("/users/{user_id}/reservations/", response_model=schemas.Reservation)
def create_reservation_for_user(
    user_id: int, item: schemas.ReservationCreate, db: Session = Depends(get_db)
):
    return crud.create_user_reservation(db=db, item=item, user_id=user_id)
